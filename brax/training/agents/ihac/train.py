...
from brax.training.agents.ihac import checkpoint
from brax.training.agents.ihac import losses as ihac_losses
...
def _init_training_state(
    ...
    ihac_network: ihac_networks.IHACNetworks,
    ...
    policy_params = ihac_network.policy_network.init(key_policy)
    policy_optimizer_state = policy_optimizer.init(policy_params)
    value_params = ihac_network.value_network.init(key_value)
...
def train(
    ...
    network_factory: types.NetworkFactory[
              ihac_networks.IHACNetworks] = ihac_networks.make_ihac_networks,
    ...
    assert num_evals_after_init >= num_unroll_lengths

    num_constant_unroll_iterations = num_evals_after_init // num_unroll_lengths
    num_evals_after_init = num_constant_unroll_iterations * num_unroll_lengths
    ...
    logging.info(
        f'Number of evaluations after initial evaluation {num_evals_after_init}'
        f'Number of constant unroll iterations per epoch {num_constant_unroll_iterations}'
        f'Number of training steps per epoch: {num_training_steps_per_epoch}'
    )
    ...
    ihac_network = network_factory(
        environment.observation_size,
        environment.action_size,
        preprocess_observations_fn=normalize)
    make_policy = ihac_networks.make_inference_fn(ihac_network)
    ...
    critic_loss, actor_loss = ihac_losses.make_losses(
        ihac_network=ihac_network,
        env=env,
    ...
    def training_step(
        ...
        env_step_per_training_step = unroll_length * action_repeat * batch_size * num_minibatches
    ...
    def training_epoch(
        ...
        def f(carry, unused_t):
            ...
            ts, es, metrics = training_step(unroll_length, ts, es, k)
    ...
    def training_epoch_with_timing(
        ...
        (training_state, env_state, metrics) = training_epoch(
            unroll_length, training_state, env_state, key
        )
    ...
    for it in range(num_evals_after_init):
        ...
        (training_state, env_state, training_metrics) = (
            training_epoch_with_timing(
                training_state, env_state, unroll_length, epoch_keys
            )
        )
