...
from brax.training.agents.hybrid_apg import networks as hybrid_apg_networks
...
def network_config(
    ...
    network_factory: types.NetworkFactory[Union[hybrid_apg_networks.HybridAPGNetworks]],
...
def _get_hybrid_apg_network(
    config: config_dict.ConfigDict,
    network_factory: types.NetworkFactory[hybrid_apg_networks.HybridAPGNetworks],
) -> hybrid_apg_networks.HybridAPGNetworks:
...
def load_policy(
    ...
    network_factory: types.NetworkFactory[
        hybrid_apg_networks.HybridAPGNetworks
    ] = hybrid_apg_networks.make_apg_networks,
  ...
  params = load(path)
  apg_network = _get_hybrid_apg_network(config, network_factory)
  make_inference_fn = hybrid_apg_networks.make_inference_fn(apg_network)
